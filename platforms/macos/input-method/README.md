...
中英文模式记忆策略，还可以选择紧凑候选窗竖排或横排，并将拼音每页候选设置为 1–9 个，
默认使用竖排和 7 个候选。横排总宽度超过候选窗上限或当前显示器可用宽度时，会自动回退
为竖排；全词候选的排列和键盘导航会跟随实际采用的紧凑排列。打开设置前会取消尚未
提交的组合并关闭候选窗，不会把半成品拼音写入客户端。设置会立即保存；输入方案和每页
候选数量会在返回文本客户端时通过 C ABI 同步到对应 librime session。若页大小在组合期间
发生变化，则保留当前页，完成当前拼音后再应用新数量，无需重启。
