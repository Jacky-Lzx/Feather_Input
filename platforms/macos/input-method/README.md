...
普通紧凑候选窗保留 84 pt 的最小宽度，刚好容纳编号和两个汉字，并减少短候选集合变化
造成的反复收缩；
全词候选窗和独立 AI 推荐窗仍按各自内容测量。

“AI 排序候选数”独立于“拼音每页候选”，范围为 1–64，默认 7。模型可以比较更多
Rime 候选，但紧凑候选窗仍只显示当前每页数量；排序数量越大，响应越可能超过采用时限。

“根据上下文和拼音生成新词（实验）”默认开启。关闭后会立即取消尚未完成的本地 MLX
请求并隐藏 AI 候选；普通 Rime 候选不受影响。重新开启后从下一次有效拼音组合开始请求。
它可以与“使用 AI 重排 Rime 候选”同时运行：重排结果更新主候选窗，生成结果独立显示在
侧边 AI 候选窗；关闭其中一项只会取消对应请求。

“上屏后显示 AI 续写候选（实验）”默认关闭。开启后，文字经 Feather 上屏并结束当前组合，
输入法会等待 400 ms，再根据最近 80 个上下文字符显示最多 3 个本地 MLX 续写候选。该窗口
显示“AI 续写”，候选无编号；可以点击，或使用 `Option + 1/2/3` 与 `Option + Space`
插入。续写窗可见时这些快捷键由输入法优先消费，不会触发欧路词典；其他新按键、选区或
前台客户端变化都会取消旧请求。插入后不会自动继续生成。
设置窗口会在后台检查本机 `127.0.0.1:1235` 的 MLX 后端状态；探测不会携带输入内容，
也不会阻塞按键处理。

Codex 编辑器可能把已提交文字后的选区位置报告为 `0`。在该客户端中，输入法会保留本次
会话内由自己提交的最近上下文，避免在下一次组合开始前误清空 MLX 生成上下文。
