...
final class AcceptanceWindowController: NSWindowController {
  ...
  private func buildInterface(in window: NSWindow) {
    ...
    let instructions = label(
      "先从菜单栏切换到 Feather Rust Dev。用全拼输入 shijie、用小鹤双拼输入 uijp，确认都能得到“世界”；输入 shi 检查全词候选。用 Caps Lock、右 Control 或 Control + Shift + Space 检查中英文切换。这个应用不链接 Rust 或 librime。",
    ...
    let checklistItems = [
      ...
      "候选字号会同步改变文字、行高和窗口尺寸",
      "主屏幕左下角常驻中英提示会同步模式，并随输入法选择和设置隐藏",
