        ...
        ScrollView {
            Form {
                ...
                Text("开启后，光标旁的中英文提示保持显示，直到开始按键或失焦；关闭后显示 0.8 秒。下次获得焦点生效。")
                    .font(.footnote).foregroundStyle(.secondary)
                Stepper("焦点提示显示：\(focusModeDuration, specifier: "%.1f") 秒", value: $focusModeDuration, in: 0.1...5.0, step: 0.1)
                    .disabled(focusModeUntilInput)
                Text("仅在关闭“持续到开始输入”时生效，范围 0.1–5.0 秒。")
                    .font(.footnote).foregroundStyle(.secondary)
                ...
                Text("优先于下一 token 排序，但不关闭上屏后补全：默认停顿 120 ms 后给当前页 Rime 候选评分，支持多 token 词。默认 700 ms 内返回才更新；可在候选对比调试面板调整。继续输入或选择会取消旧请求，后端繁忙时仅重试最新请求。侧窗名次是候选评分排名。")
                    .font(.footnote).foregroundStyle(.secondary)
                Toggle("用 MLX 下一 token 给拼音候选排序", isOn: $rerankingEnabled)
                Text("未开启候选评分时，上屏后后台预测，下一轮拼音固定使用已缓存结果。当前页精确匹配的候选优先，未匹配项保持原顺序；无结果时沿用 Rime。若同时开启 MLX 上屏补全，同一次预测也会显示补全候选。")
                    .font(.footnote).foregroundStyle(.secondary)
