...
final class PredictionPanel {
    ...
    func show(_ tokens: [LocalRecommendation.RankedToken], beside anchor: NSRect, visible: NSRect) {
        ...
        label.stringValue = (["LLM top-k · 本轮预测", ""] + rows).joined(separator: "\n")
        let width: CGFloat = min(300, max(190, rows.map { ($0 as NSString).size(withAttributes: [.font: label.font!]).width + 36 }.max() ?? 190))
        let height = CGFloat(rows.count + 2) * 18 + 24
