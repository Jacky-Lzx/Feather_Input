...
它只连接 `127.0.0.1`，把上下文、规范化拼音预编辑、输入方案和最多 3 个结果的数量发送给独立
后端。Provider 返回原 request ID 和输入 revision；异步任务与 C ABI 轮询接口会再次核对
这两个值，不得采用来自旧组合的响应。阻塞调用只在 Rust 工作线程执行，平台按非阻塞方式
轮询。macOS 控制器已经接入独立推荐窗；请求失败或身份过期时静默保留 Rime 路径，生成
候选使用专用快捷键或点击提交，不参与 Rime 候选排序。
...
主屏幕左下角的常驻“中／英”提示也属于 macOS 呈现策略：每个输入控制器只持有带所有权
令牌的代理，进程内共享一个面板。激活控制器时转移所有权并同步核心的 direct mode，切换
模式时更新，停用输入法、安全输入或设置关闭时隐藏，从而避免多客户端重复创建常驻窗口。
