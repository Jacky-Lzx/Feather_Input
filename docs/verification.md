...
## Experimental LM Studio recommendations

Added a loopback-only, ephemeral URLSession client for model discovery and structured candidate-ID selection, with redirects disabled, 2-second request / 3-second resource timeouts and a 32-token output budget. Prompts request non-thinking output; model IDs are discovered from the authenticated local API, not assumed from filenames. Response parsing rejects malformed, noninteger and out-of-range selections.

The controller waits 250 ms without input, then submits a snapshot. Version, cancellation, activation, current preedit/candidates and enabled/model settings gate results. Rime immediately renders its usual candidates; a star marks a recommendation in reserved space without changing order, numeric identity, highlight, or Space behavior. Recent context is limited to this controller's own last 80 committed characters and cleared on navigation, modifier changes, composition commit, activation/deactivation and mode changes. It does not query document text. Programmatic edits without IMK events cannot be reliably detected by this initial context heuristic.

Settings default AI off and provide model discovery, a synthetic inference check, and a secure token field backed by Keychain. Input-time credential reads prohibit authentication UI. Tests cover request context bounds, token placement, malformed or invalid IDs, and an injected delayed model reply: valid results are marked, cancelled replies ignored, candidate order preserved, and deactivation invalidates results. Both real schema engine checks and isolated packaged smoke checks passed. A standalone settings-view bitmap was inspected for layout; system-composited control surfaces are not faithfully captured by this bitmap method.
