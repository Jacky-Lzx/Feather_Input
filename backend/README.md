...
设置中的「根据上下文和拼音生成新词（实验）」默认开启。侧边单独显示 `AI · 词语`，点击替换本轮**全部未上屏拼音**；没有 Rime 候选 ID，不套用 Rime 排名先验，也不占用数字键。与当前可见页及已送评 Rime 列表重复的词不在 AI 侧窗重复显示；调试面板仍保留后端全部生成结果。生成结果不加入展开的 Rime 候选网格。
