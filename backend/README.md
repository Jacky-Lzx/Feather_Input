...
开启 **用 MLX 给当前拼音候选打分**。它的优先级高于缓存 top-k 排序、独立补全和旧版推荐。Rime 根据全拼或小鹤双拼提供符合输入的候选，默认取当前页；开启自定义发送数量时，从当前页起按原始顺序取指定数量，模型不直接解析原始拼音。`POST /score` 接收 `context`、`preedit`（用于调试的元数据）以及 1–64 个候选字符串，每个候选最多 64 个字符，分词后也不能超过 64 个 token。
