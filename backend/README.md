...
全部候选窗口的第一列保留展开前已生效的当前页显示顺序（包括 LLM 重排）；其他候选按 Rime 原始全局顺序接续，排除第一列已有的全局候选索引。展开期间不再应用新返回的 LLM 排序；操作说明见 [主文档](../README.md#全部候选窗口)。

### 独立配置发送给 LLM 的候选数

在候选对比调试面板开启 **自定义发送候选数**，可设置 1–64 个（自定义初始值为 20）。关闭时跟随当前 Rime 每页候选数量。配置保存在本机，与记录开关和窗口是否打开无关。

评分请求从当前页的起始位置取候选，按 Rime 原始顺序延伸到后续页；不足指定数量时使用实际数量。主窗口仍只重排当前页，未参与评分的当前页候选保持原相对顺序排在已评分项之后，不自动把后续页候选提前。调试面板显示送评的 Rime 列表、LLM 排名和当前实际显示列表，并标注各自数量。修改发送数量会使旧配置下的响应失效。
