# Feather MLX 后端（原型）

仅支持 Apple Silicon。模型在独立进程中运行，HTTP 服务只监听 `127.0.0.1:1235`，无需 API key。后端不会记录输入文本，也不会将其持久化到磁盘。

## 安装与启动

在仓库根目录执行：

```sh
uv venv .venv-mlx
uv pip install --python .venv-mlx/bin/python -r backend/requirements.txt
scripts/start-mlx.sh /absolute/path/to/MLX-model
```

将 `/absolute/path/to/MLX-model` 替换为模型的绝对路径。若需登录后自动启动，而不是在前台运行：

```sh
python3 scripts/install-mlx-agent.py /absolute/path/to/MLX-model
```

LaunchAgent 指向当前仓库和虚拟环境，请保留两者的位置。卸载时先执行 `launchctl bootout gui/$(id -u)/im.feather.mlx-worker`，再删除 `~/Library/LaunchAgents/im.feather.mlx-worker.plist`。

## 上屏后预测下一个 token

在 Feather 设置中将补全后端选为 **MLX 下一 token**，并开启 **上屏后 AI 预测**。候选推荐仍有独立的 LM Studio 选项。MLX 测试按钮使用合成文本；在请求前开启调试模式，可以查看上下文、下一 token 的排名及其对数概率、概率，以及按配置数量筛选出的候选。点击候选会插入对应文本；继续输入会取消界面请求，过期响应会被丢弃。

算法调用一次 `generate_step(..., max_tokens=1)`，读取未经修改的下一 token 概率分布。它不展开后续分支、不生成词组、不裁剪空白，也不移除重复上下文。从较大的原始概率候选池中筛选出配置数量的、互不重复且可显示的 token（1–20 个），按概率排列，保留标点和空格。候选标签中的空格显示为 `␠`，插入时仍是原始空格。

特殊 token、控制字符和不完整的 Unicode 片段不会成为可选候选；原始概率值仍可在调试输出中查看。每个候选恰好对应一个 token ID 及其模型对数概率。概率受量化和数值精度影响，前十个 token 的概率之和不一定为 1。此预测模式没有拼音约束，也尚未迁移到 MLX Swift。聊天模型的直接续写效果可能不理想；程序不会强制输出逗号。

正在执行的 Metal 运算不能在单步中途终止。后端忙碌时直接拒绝新请求、不排队；应用会对仍然有效的最新请求做有限、可取消的退避重试。应用请求超时为 3 秒，并丢弃过期响应。目前没有跨请求的持久 KV 缓存。模型在服务开始监听前完成加载。后端不可用不会阻塞 Rime 的按键处理。

接口如下：

- 健康检查：`GET /health`。
- 预测：`POST /continuations`，JSON 包含 `context`（1–80 个字符），以及可选整数 `count`（1–20，默认 5）。

服务拒绝带浏览器 Origin 的请求。本机其他进程仍可访问这个回环地址，因此它不构成身份认证边界。

## 用下一 token 的 top-k 结果重排 Rime 候选

开启 **用 MLX 下一 token 给拼音候选排序**。此模式的优先级高于独立预测弹窗和旧版 LM Studio 推荐。文字上屏后，应用立即在后台请求 top-k token；开始下一段小写拼音时，冻结已经就绪的预测缓存，没有结果则冻结为空。后续返回的结果不能移动本轮候选。预测仅属于当前输入上下文，切换输入源或焦点会使其失效。

在每一页 Rime 候选中，与预测 token **文本完全相同**的候选按模型概率顺序提前；未匹配项和同等匹配项保持 Rime 原顺序。不做前缀或子串匹配，不新增候选，也不跨页提升候选。空格、数字键和鼠标点击会映射回 Rime 原候选索引；上下键在重排后的页面中选词，右键打开全部候选窗口，展开后左右切列、上下选行；普通窗口可用 Page Up/Down 和 Fn + ↑ / ↓ 翻页。按键处理不会等待模型推理。

主候选窗口只显示选词序号和文本，模型排名只在侧窗显示；排序和选词映射不受影响。排名信息不会插入正文。

Rime 候选显示期间，只读侧窗展示本轮排序使用的完整冻结预测列表，包括未匹配到 Rime 候选的 token。列表按原始 LLM 名次排列，空格显示为 `␠`。侧窗跟随主候选窗口，右侧空间不足时移到左侧，并随主窗口一起隐藏。长列表可滚动，不抢占键盘焦点。本轮没有就绪预测时，不显示侧窗。

侧窗还显示以毫秒计的请求往返延迟。计时使用单调时钟，覆盖请求及响应解析，包括本机传输和推理，并与预测结果一起冻结。它不包含用户开始下一段输入前的等待时间，也不是纯模型计算耗时。

## Rime 候选概率评分与融合排序（实验性）

开启 **用 MLX 给当前拼音候选打分**。它的优先级高于缓存 top-k 排序和旧版推荐，但上屏后的独立补全仍会运行。Rime 根据全拼或小鹤双拼提供符合输入的候选，默认取当前页；开启自定义发送数量时，从当前页起按原始顺序取指定数量，模型不直接解析原始拼音。`POST /score` 接收 `context`、`preedit`（用于调试的元数据）以及 1–64 个候选字符串，每个候选最多 64 个字符，分词后也不能超过 64 个 token。

### 评分方法

对每个候选，后端分别编码上下文和候选，再按 `encode(context) + encode(candidate)` 拼接，明确两者的 token 边界。计算时使用候选已有的 token 作为后续位置的输入，逐个读取条件对数概率，只累计候选位置的分数。

例如候选由两个 token 组成，其总对数概率为：

```text
log P(token₁ | 上下文) + log P(token₂ | 上下文, token₁)
```

响应包含原候选 ID、token ID、各 token 的条件对数概率、总对数概率、归一化 LLM 分数、Rime 排名分数和最终融合分数。设置中可调整 LLM 权重（0–100%，默认 35%）及长度处理方式：按字符平均、按 token 平均或不归一化，默认按字符平均。

```text
score = (1 - weight) * -log(original_rank) + weight * normalized_logprob
```

其中 `original_rank` 是从 1 开始的 Rime 原始排名，`log` 为自然对数，`normalized_logprob` 是按所选方式处理后的 LLM 分数。最终分数越高，排名越靠前；同分时保持原顺序。

Rime 项只是根据排名构造的分数，**不是 Rime 的真实概率**。权重为 0 时保留 Rime 顺序，为 1 时完全使用 LLM 分数。默认值是初始启发式参数，不是拟合或校准得到的最优值。归一化可能偏向较长、容易预测的字符串；评分不包含结束标记 EOS 的概率。

当前使用的 Rime API 不提供候选实际消耗的拼音范围，因此不能保证参与比较的候选覆盖相同范围。严格的拼音范围分组和跨页提词尚未实现。

### 上下文与缓存

上下文最多 80 个字符。每个请求只对上下文做一次预填充，复用首个 token 的概率分布；对每个包含多个 token 的候选，深拷贝上下文 KV 缓存后分别计算。候选分支不会修改彼此的前缀缓存。目前没有持久缓存，也不跨请求复用。

开始新一段拼音前，应用通过当前输入应用的 IMK 文本接口，读取选区起点前最多 80 个字符；接口查询使用有界的 UTF-16 范围。这样，移动光标或删除正文后，可以根据更新后的文档恢复评分，而不只是依赖输入法最近上屏的文字。选中文本、光标后的文本及本轮待上屏拼音不计入上下文。

若应用不提供光标周围的正文，仍回退到本地上屏历史；移动位置会清空该历史，因此这类应用中无法恢复上下文。此功能不使用辅助功能权限，也不读取其他应用。旧版缓存 next-token 模式仍要求开始拼音前已有预测结果，不会在此处等待新请求。

### 请求时机与界面更新

应用先立即显示 Rime 当前页候选，默认输入停顿 120 ms 后再请求评分。默认只有请求开始后 700 ms 内返回，并且页面及上下文仍未变化的响应，才能用于重排。继续输入、导航、选择、焦点变化、关闭相关设置或取消请求，都会使旧响应失效。鼠标停留在主候选窗口或侧窗上时，也不会应用待返回的排序；用上下键选词会锁定当前页评分。

与冻结 next-token 预测的模式不同，此模式允许静止页面在评分返回后更新一次。侧窗标题为 **Rime + LLM · 融合排序**，显示的是所提交候选之间的融合排名，**不是整个模型词表的 token 排名**。请求延迟仍会显示；开启调试窗口可查看各 token 的分数。

请求超时、服务忙碌、响应无效或缺少上下文时，Rime 仍可正常使用。后端在候选之间检查 2.4 秒的处理预算，单次 Metal 运算无法中途打断。此模式不会跨页排序，也不会自动插入文字。关闭该选项后，如果缓存 top-k 排序选项仍开启，就会回到旧模式。

## 可复现的诊断评测

`evaluation_cases.json` 包含 20 个人工编写的“上下文 / 拼音 / 目标词”样例，其中包括同音词对。先使用隔离的用户目录，导出 Rime 实际生成的首页候选（每页 9 个），再评测已安装的模型：

```sh
scratch=$(mktemp -d /tmp/feather-eval.XXXXXX)
.build/release/EngineCheck "$PWD/dist/FeatherInput.app/Contents/Frameworks/librime.dylib" \
  "$PWD/dist/FeatherInput.app/Contents/Resources/rime" "$scratch" \
  backend/evaluation_cases.json > /tmp/feather-evaluation-candidates.json
.venv-mlx/bin/python backend/evaluate.py \
  --model "$HOME/Library/Application Support/FeatherInput/models/Qwen3-1.7B-Base-MLX-8bit" \
  --fixtures /tmp/feather-evaluation-candidates.json --output /tmp/feather-evaluation-results.json
```

评测报告包含原始 Rime、纯 LLM 按 token / 字符平均评分、35% 融合排序的 Top-1 和 Top-5 命中数。所有样例都计入分母，目标词不在候选页中也算未命中。同时报告目标词覆盖数、预热后每次请求使用缓存与不使用缓存的耗时中位数和 P95（最近秩法），以及逐 token 分数的最大差异。

计时不包含 HTTP 和界面处理；每个样例先运行缓存版本，再运行无缓存版本，只用于诊断。这组数据是人工合成的全拼样例，不含个人学习记录，不是独立留出的基准测试，也不能证明实际使用准确率。没有使用它调整权重。

### 已记录结果

详见 [2026-09-14 评测结果](evaluations/2026-09-14-qwen17-fusion.json)。这 20 个样例的结果如下：

| 排序方法 | Top-1 命中数 | Top-5 命中数 |
| --- | --- | --- |
| Rime 原顺序 | 11/20 | 18/20 |
| 纯 LLM，按 token 平均 | 20/20 | 20/20 |
| 纯 LLM，按字符平均 | 20/20 | 20/20 |
| 35% 融合排序 | 19/20 | 20/20 |

因此，这组样例**没有证明融合排序优于纯 LLM 评分**。

缓存版本的耗时中位数 / P95 为 41.5 / 88 ms，无缓存版本为 85 / 109 ms。BF16 计算产生了部分分数差异，token 对数概率的最大绝对差为 0.13455：20/20 个样例的首选一致，18/20 个样例的完整顺序一致。单个 FP32 对照样例将最大差异降至 0.00000763；不能认为缓存前后的计算结果逐位相同。

## 候选对比调试面板

从输入法菜单选择 **候选对比调试…**，或在设置中点击 **打开候选对比调试面板**。打开面板会启用候选记录，三列分别展示：

- **Rime · 原始排名**：已采用评分请求实际送评的候选顺序；等待结果时显示当前页原始候选。
- **LLM · 模型排名**：候选评分模式下，按后端返回的 `lm_score` 排列并显示纯模型分数；旧版 top-k 模式下，显示冻结的 token 及其原始名次。
- **最终候选 · 显示顺序**：实际显示给用户的候选顺序；候选评分模式还显示融合后的 `score`。

面板显示拼音、快照时间及已采用请求的往返延迟。尚无已采用模型结果时，LLM 列为空，最终列保留 Rime 顺序。旧后端未提供 `lm_score` 时，也不会把融合分数冒充纯模型分数。

面板只观察当前模式已采用的结果，不额外请求其他后端，也不展示被取消或丢弃的响应；原始请求及响应仍可在 LLM 调试窗口查看。只保留最近一次候选快照，上屏后仍保留供检查，可暂停更新或手动清空。关闭记录会清空，关闭窗口仍继续记录；记录仅存于内存，不写入磁盘。面板使用不激活应用的浮动窗口，打开时不抢走当前输入焦点。

### 调试评分时序

在候选对比调试面板开启 **自定义评分时序**，即可调整：

| 配置项 | 默认值 | 可调范围 | 含义 |
| --- | --- | --- | --- |
| 请求前停顿 | 120 ms | 0–2000 ms，每步 10 ms | 最后一次输入后等待多久才发送评分请求；继续输入会重新计时 |
| 响应采用时限 | 700 ms | 50–2000 ms，每步 50 ms | 从发出请求到完成响应解析允许的最大耗时，不包含前面的停顿 |

响应采用时限决定是否使用结果，不会强行中断 Metal 推理，也不修改 HTTP 超时或后端处理预算。输入变化、导航、鼠标悬停等原有失效条件仍然适用。设置仅影响当前拼音候选评分模式，不影响旧版 next-token top-k 或独立补全。

设置保存在本机，关闭面板后仍生效，与是否记录候选无关。关闭 **自定义评分时序** 或点击 **恢复默认** 后，实际使用 120 / 700 ms。修改有效时序参数会使旧请求失效，下一次候选评分使用新参数。

全部候选窗口将当前输入全部已采用的评分候选按融合分数排列在前面，可跨越多列；未评分候选按 Rime 原始全局顺序接续，排除已包含的全局索引。没有有效评分时，保留展开前当前页显示顺序作为开头。展开期间不再应用新返回的 LLM 排序；操作说明见 [主文档](../README.md#全部候选窗口)。

### 独立配置发送给 LLM 的候选数

在候选对比调试面板开启 **自定义发送候选数**，可设置 1–64 个（自定义初始值为 20）。关闭时跟随当前 Rime 每页候选数量。配置保存在本机，与记录开关和窗口是否打开无关。

评分请求从当前页的起始位置取候选，按 Rime 原始顺序延伸到后续页；不足指定数量时使用实际数量。主窗口仍只重排当前页，未参与评分的当前页候选保持原相对顺序排在已评分项之后，单列窗口不自动把后续页候选提前；展开窗口会把全部已评分候选按融合分数提前。调试面板显示送评的 Rime 列表、LLM 排名和当前实际显示列表，并标注各自数量。修改发送数量会使旧配置下的响应失效。

`POST /score` 支持最多 64 个候选，请求体上限为 64 KiB；单个候选仍限制为 64 个字符 / token。增加数量可能触及响应采用时限或后端 2.4 秒处理预算，失败时保留 Rime 顺序。此配置不改变旧版 next-token 模式的 top-k 数量。

## 按拼音生成新词（实验）

`POST /generate` 从「光标前上下文 + Rime 原始按键串 + 方案」生成新候选，不接收 Rime 候选列表。现有 `/score` 继续负责给已有词打分，两个通道独立。

```json
{"context":"落霞与","input":"guwu","scheme":"luna_pinyin_simp","count":3}
```

返回示例（分数为生成 token 的原始条件对数概率之和）：

```json
{"candidates":[{"text":"孤鹜","score":-0.4715,"token_ids":[100548,121535]}],"syllables":[["gu","wu"]],"elapsed_ms":359,"truncated":false}
```

实现位于 `pinyin_generation.py`：

1. 使用 pypinyin 0.55.0 的单字读音表，不使用它的词组库生成候选。全拼按完整音节切分，空格和撇号是硬分界；最多保留 4 条歧义路径。小鹤按当前内置 Rime 方案的转换规则反查双键编码，包含零声母和 `jv/qv/xv/yv` 别名。
2. 把每个音节允许的汉字编成 UTF-8 字节约束，与 Qwen ByteLevel BPE 的 token 字节树求交。一个 token 可以包含多个汉字，也可以只包含一个汉字的部分字节；字节未齐时不会把生僻字直接过滤掉。
3. 上下文只预填一次，每条搜索分支独立复制 KV cache。束宽 6，按完整模型分布的 logprob 扩展（没有在允许 token 集内重新归一化）；完成输入音节后停止。最终再检查整段文字的逐字读音和长度，去重后返回最多 3 个结果。
4. 搜索预算 1.8 秒，在推理调用之间检查，不能中断正在运行的 Metal 调用。达到预算时 `truncated=true`，只返回已完整生成的候选，可能为空。束搜索是近似搜索，不保证全局最优。

客户端在 Rime 评分请求结束后启动生成，避免两种请求争抢同一个后端锁；生成前沿用调试中的停顿时间。生成不受评分的 700 ms 采用时限限制，HTTP 请求超时 2.5 秒（资源总时限 3 秒）。本地服务仍只监听 loopback、拒绝浏览器 Origin、不写入输入日志。

设置中的「根据上下文和拼音生成新词（实验）」默认开启。侧边单独显示 `AI · 词语`，按 `⌥ Option + 1 / 2 / 3` 或点击替换本轮**全部未上屏拼音**；没有 Rime 候选 ID，不套用 Rime 排名先验，也不占用数字键。与当前可见页及已送评 Rime 列表重复的词不在 AI 侧窗重复显示；调试面板仍保留后端全部生成结果。生成结果不加入展开的 Rime 候选网格。

继续输入、删除、移动光标、切换输入状态、关闭当前组合或进入安全输入时，取消请求并隐藏建议。返回和点击时再次核对请求版本、原始输入、上下文、输入方案、选区和前台应用。已经部分选词形成汉字的组合不生成建议，避免把已选部分错误覆盖。

首版仅支持有前文的完整 2–6 音节输入，暂不支持首字母缩写、模糊音和末尾未完成的音节。读音表包含多音、异读及繁体，所以读音匹配不代表词义正确，也不保证只出现简体；是否采用由用户决定。模型无需知道整个复合词已存在于词库。

2026-09-14 在当前 Qwen3-1.7B-Base-MLX-8bit 后端的真实 HTTP 检查中：

| 前文 | 输入 | 方案 | 第一候选 | 后端耗时 |
|---|---|---|---|---|
| 落霞与 | guwu | 全拼 | 孤鹜 | 359 ms |
| 这句话让我很受 | guwu | 全拼 | 鼓舞 | 115 ms |
| 落霞与 | guwu | 小鹤双拼 | 孤鹜 | 177 ms |

这是三次功能检查，不是延迟分位数或正确率评估；端到端还包含停顿、已有候选评分和 HTTP 往返。

读音库及接口说明：[python-pinyin](https://github.com/mozillazg/python-pinyin)。拼音约束生成的研究参考：[Exploring and Adapting Chinese GPT to Pinyin Input Method](https://aclanthology.org/2022.acl-long.133/)。本项目使用冻结的 Qwen 和字节约束搜索，没有复现论文训练流程。
